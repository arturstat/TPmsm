survTP <- function(time1, event1, Stime, event, ...) {
  ...
  Message <- dataCheck(
    ...
    names=c("time1", "event1", "Stime", "event"),
    arg=TRUE
  );
  if ( !is.null(Message) ) stop(Message);
  data <- list(
    "time1"=as.double(time1),
    "event1"=as.integer(event1),
    "Stime"=as.double(Stime),
  ...
  if (datalen > 4) {
    ...
    for (i in 5:datalen) {
      ...
      if ( length(data[[i]]) != length(time1) )
        stop(
          "All additional arguments must have the same length as arguments ",
          "'time1', 'event1', 'Stime' and 'event'"
        );
