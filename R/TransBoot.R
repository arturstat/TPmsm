...
TransBoot.TP <- function(
  ...
  TPmsm <- BtoTPmsm(
    lst, UT, s, t, state.names, n.boot, conf.level, method.boot
  );
  Clean(object);
  return(TPmsm);
} # TransBoot.TP
...
TransBoot.TPC <- function(
  ...
  class(lst) <- class(object)[2];
  TPCmsm <- BtoTPCmsm(
    lst, UT, UX, s, t, xi, state.names, n.boot, conf.level, method.boot
  );
  Clean(object);
  return(TPCmsm);
} # TransBoot.TPC
