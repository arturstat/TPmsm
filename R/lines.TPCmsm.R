lines.TPCmsm <- function(
  x, plot.type="t", tr.choice, col, lty, conf.int=FALSE, ci.col, ci.lty,
  ...
  if (plot.type == "t") {
    ...
    for ( i in seq_len(lt) ) {
      for ( j in seq_len(lx) ) {
        lines(
          x=x$time,
          y=x$est[,which(x$covariate==x$x[j]),itr[i]],
          type="s",
          col=col[j+lx*(i-1)],
          lty=lty[j+lx*(i-1)],
          ...
    ...
    if (plot.type == "t") {
      ...
      for ( i in seq_len(lt) ) {
        for ( j in seq_len(lx) ) {
          lines(
            x=x$time,
            y=x$inf[,which(x$covariate==x$x[j]),itr[i]],
            type="s",
            col=ci.col[j+lx*(i-1)],
            lty=ci.lty[j+lx*(i-1)],
            ...
          ...
          lines(
            x=x$time,
            y=x$sup[,which(x$covariate==x$x[j]),itr[i]],
            type="s",
            col=ci.col[j+lx*(i-1)],
            lty=ci.lty[j+lx*(i-1)],
            ...
