lines.TPmsm <- function(
  x, tr.choice, col, lty, conf.int=FALSE, ci.col, ci.lty,
  ...
  lt <- length(tr.choice);
  if (sum( tr.choice %in% colnames(x$est) ) != lt)
  ...
  if ( conf.int & !is.null(x$inf) & !is.null(x$sup) ) {
    ...
    for ( i in seq_len(lt) ) {
      lines(
        x=x$time, y=x$inf[,itr[i]], type="s", col=ci.col[i], lty=ci.lty[i], ...
      );
      lines(
        x=x$time, y=x$sup[,itr[i]], type="s", col=ci.col[i], lty=ci.lty[i], ...
      );
    }
  }
  if (legend) {
