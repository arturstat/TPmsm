image.TPCmsm <- function(
  x, image.type="tc", tr.choice, xlim, ylim, zlim=c(0, 1), col, xlab, ylab,
  main, sub, key.title, key.axes, las=1, conf.int=FALSE, legend=TRUE,
  curvlab, contour=TRUE, nlevels=20, levels=pretty(zlim, nlevels), ...
) {
  if ( !inherits(x, "TPCmsm") ) stop("'x' must be of class 'TPCmsm'");
  if ( !( image.type %in% c("tc", "ct") ) )
    stop("Argument 'image.type' must be one of 'tc' or 'ct'");
  ...
  if (image.type == "tc") {
    ...
    if ( missing(ylim) )
      ylim <- c(x$covariate[1], x$covariate[length(x$covariate)]);
    for ( i in seq_len(lt) ) {
      image(
        x=x$time,
        y=x$covariate,
  ...
  plot.new();
  title(
    main=main, sub=sub, xlab=xlab, ylab=ylab, cex.main=1.2, cex.lab=1.2, ...
  );
  invisible();
} # image.TPCmsm
